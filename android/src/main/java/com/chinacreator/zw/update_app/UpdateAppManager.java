...
import com.chinacreator.zw.update_app.listener.ExceptionHandler;
import com.chinacreator.zw.update_app.listener.ExceptionHandlerHelper;
...
//                    .show(((FragmentActivity) mActivity).getSupportFragmentManager(), "dialog");
            UpdateDialog.newInstance(mActivity).updateApp(mUpdateApp).show();
        }
    ...
    public void update(boolean isManual) {
        checkNewApp(new UpdateCallback(mActivity,isManual));
    }
    ...
    public void checkNewApp(final UpdateCallback callback) {
        ...
        if (DownloadService.isRunning || UpdateDialog.isShow) {
            callback.onAfter();
        ...
        if (isPost) {
            ...
            mHttpManager.asyncPost(mUpdateUrl+"/appVersion/checkoutNewAppByOrgId", params, headers,new HttpManager.Callback() {
                @Override
            ...
            Map<String,String> headers = new HashMap<>();
            headers.put("x-api-key",mApiKey);
            mHttpManager.asyncGet(mUpdateUrl+"/appVersion/checkoutNewAppByOrgId", params,headers, new HttpManager.Callback() {
                @Override
                public void onResponse(String result) {
                    callback.onAfter();
                    if (result != null) {
    ...
    private void processData(String result, @NonNull UpdateCallback callback) {
        try {
            String versionName = AppUpdateUtils.getVersionName(mActivity);
            mUpdateApp = callback.parseJson(result,mUpdateUrl,versionName);
    ...
    public static class Builder {
       ...
       // private Map<String, String> headerParams;
        ...
        public Builder setActivity(Activity activity) {
            mActivity = activity;
            mHttpManager = new UpdateAppHttpUtil();
            return this;
        }
